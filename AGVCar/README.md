...
To build the project, easiest is with the vscode built in build function for ESP-IDF.
...
* If the camrea init failsdecrease log level to info, adn increase ram to 4mb

### Tasks and issues

SPI comminication is currently not wokring in both frameworks.
The communication should be fixed or UART should be used.
We have the communcations set up curretcly, there are no errors thrown, but the transaction never happens.
...
Code for the nucleo is well constructed, and msotly autogenerated by the Cube IDE. Information from the Nucle can be found [here](https://www.st.com/en/evaluation-tools/nucleo-f746zg.html).
...
* **ConnectionTrainer** is a bad wrapper for the netowkrs, should be removed.
  
# Neural Networks
The project uses reinforcment learning.
The netwokrs are trained with an epsilon greedy straegy.
There are couple networks defiend in the *Networks* folder, each of them are created with the size of the image and the number of addition inputs, suchs as speed and acceleration.
The **ReinforcmentModel** is the training model.
It laods the network and performs training.
The traing data is stored in a **ReplayMamory**.
