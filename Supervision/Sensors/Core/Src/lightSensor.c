...
uint16_t SPIreceive_AD(uint16_t idx, uint8_t AD_Num)
{
	...
	//TEST CASE
	uint8_t buff_out[2];
	buff_out[0] = (uint8_t)(idx >> 8);
	buff_out[1] = (uint8_t)(0x00FF & idx);
...
void lightSensorCycle()
{
	...
	for(int ledGroup = 0; ledGroup < 4; ledGroup++)
	{
		for(int ledNumInGroup = 0; ledNumInGroup < 8; ledNumInGroup++)
		{
			//if(ledGroup == 0 && ledNumInGroup == 7) //DEBUGHOZ
				printf("D%d%c: %u\n", ledNumInGroup+1, 'A'+4-ledGroup, AD_IN[8*ledGroup+ledNumInGroup]); //DEBUGHOZ
	...
	//TEST CASE
	/*{
		leds_buff[1] = leds_buff[2] = leds_buff[3] = 0;
		leds_buff[0] = 0xFF;
